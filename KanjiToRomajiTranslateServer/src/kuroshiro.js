 ...
 * @param {string} str Given String
 */
var convertToApiFormat = function (str)
{
    //make sure that has character
    str = str || '';

    //convert kapanese character to token
    var tokens = tokenizer.tokenize(str);
    for (var cr = 0; cr < tokens.length; cr++) {
        if (!tokens[cr].reading)
            tokens[cr].reading = tokens[cr].surface_form;
    ...
    for (var i = 0; i < tokens.length; i++)
    {
        ...
        switch (strType) {
            case 0:
                notations.push([tokens[i].surface_form, 1 ,0, tokens[i].reading]);
                break;
            case 1:
                var pattern = '';
                var isLastTokenKanji = false;
                ...
                if (matches) {
                    ...
                    for (var c1 = 0; c1 < subs.length; c1++) {
                        if (isKanji(subs[c1][0])) {
                            notations.push([subs[c1], 1, c1, matches[pickKanji++]]);
                        } else {
                            notations.push([subs[c1], 2, c1, wanakana.toHiragana(subs[c1])]);
                        }
                    }
                } else {
                    notations.push([tokens[i].surface_form, 1, 0, tokens[i].reading]);
            ...
            case 2:
                for (var c2 = 0; c2 < tokens[i].surface_form.length; c2++) {
                    notations.push([tokens[i].surface_form[c2], 2, c2, tokens[i].reading[c2]]);
                }
                break;
            case 3:
                for (var c3 = 0; c3 < tokens[i].surface_form.length; c3++) {
                    notations.push([tokens[i].surface_form[c3], 3, c3, tokens[i].surface_form[c3]]);
                }
                break;
        }
    }
    ...
    for (var i = 0; i < notations.length; i++) {
        var single =
        {
            Text: notations[i][0],//原本的字
            Type : notations[i][1],//是不是漢字
            Index : notations[i][2],//字的index
            Hiragana : notations[i][3],//平假名
            Katakana : wanakana.toKatakana(notations[i][3]),//片假名
            Romaji : wanakana.toRomaji(notations[i][3]),//羅馬拼音
        }
...
var kuroshiro = {
    ...
    toApiFormat: convertToApiFormat
};
